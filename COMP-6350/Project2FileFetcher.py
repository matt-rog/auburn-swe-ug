...
def extract_files():
    ...
    for file_type, signature in FOOTER_BASED_SIGNATURES.items():
        ...
        print(f"h:{len(header_offsets)}, f:{len(footer_offsets)}")
        ...
        if len(header_offsets) == file_count:
            for start_offset in header_offsets:
                ...
                for footer in footer_offsets:
                    if footer > start_offset and footer > end_offset and footer < next_header:
                        end_offset = footer
            ...
            for end_offset in footer_offsets:
                ...
                for header in header_offsets:
                    if header < end_offset and header < start_offset and header > prev_footer:
                        start_offset = header
...
def extract_and_hash(file_num, file_type, file_size, start_offset, end_offset):
    print(f"Extracting {file_num}.{file_type.lower()} ... | Start offset: {start_offset} | End offset: {end_offset}")
